...
def calculateScheduleDates():
    ...
    return dates, month_for_schedule, year_for_schedule


def createUnavailabilityList(input_file, month_for_schedule, year_for_schedule):
...
# 2. Build the model
# ------------------------------
def createAssignmentVariables(model, dates, doctors_list, unavailability):
...
def addConstraintOfOneDoctorPerDay(model, x, doctors_list, dates):
...
def addConstraintBalancedTotalDutiesPerDoctor(model, x, doctors_list, dates):
...
def addConstraintBalancedWeekendDuties(model, x, doctors_list, dates, is_weekend):
...
# ------------------------------
def addSoftConstraintPenalizeCloseTogetherDuties(model, x, doctors_list, dates):
...
def addSoftConstraintSpreadDutiesAcrossMonth(model, x, dates, doctors_list):
...
def addSoftConstraintBalanceFullWeekendsOff(model, x, doctors_list, dates):
    ...
    max_wkend_off = min_wkend_off if total_full_weekends % len(
        doctors_list) == 0 else min_wkend_off + 1
    ...
    return full_weekend_off_bonus, balanced_full_wkends_off_deviation_vars


def addSoftConstraintDifferentWeekendDuties(model, x, doctors_list, dates):
...
def addHardConstraintOneFullWeekendOff(model, x, doctors_list, dates):
    for doc in doctors_list:
...
# ------------------------------
def combineObjectives(full_weekend_off_bonus, every_other_vars, gap2_vars, block_deviation_vars, balanced_full_wkends_off_deviation_vars, different_weekend_duty_day_vars):
...
def exportToExcel(solver, x, doctors_list, dates):
    schedule = []
    for i, day in enumerate(dates):
        assigned = None
        for doc in doctors_list:
...
def printStatistics(solver, x, doctors_list, dates, is_weekend):
    print("\nDuties per doctor:")
    for doc in doctors_list:
        duties = 0
...
if __name__ == '__main__':
    print("here")
    dates, month_for_schedule, year_for_schedule = calculateScheduleDates()
    unavailability, doctors_list = createUnavailabilityList(
        INPUT_FILE, month_for_schedule, year_for_schedule)
    # Identify weekends
    is_weekend = {d: (d.weekday() >= 5) for d in dates}

    model = cp_model.CpModel()
    x = createAssignmentVariables(
        model=model, dates=dates, doctors_list=doctors_list, unavailability=unavailability)
    addConstraintOfOneDoctorPerDay(
        model=model, x=x, doctors_list=doctors_list, dates=dates)
    addConstraintBalancedTotalDutiesPerDoctor(
        model=model, x=x, doctors_list=doctors_list, dates=dates)
    addConstraintBalancedWeekendDuties(
        model=model, x=x, doctors_list=doctors_list, dates=dates, is_weekend=is_weekend)

    every_other_vars, gap2_vars = addSoftConstraintPenalizeCloseTogetherDuties(
        model=model, x=x, doctors_list=doctors_list, dates=dates)
    block_deviation_vars = addSoftConstraintSpreadDutiesAcrossMonth(
        model=model, x=x, doctors_list=doctors_list, dates=dates)
    full_weekend_off_bonus, balanced_full_wkends_off_deviation_vars = addSoftConstraintBalanceFullWeekendsOff(
        model=model, x=x, doctors_list=doctors_list, dates=dates)
    different_weekend_duty_day_vars = addSoftConstraintDifferentWeekendDuties(
        model=model, x=x, doctors_list=doctors_list, dates=dates)

    addHardConstraintOneFullWeekendOff(
        model=model, x=x, doctors_list=doctors_list, dates=dates)

    full_obj = combineObjectives(full_weekend_off_bonus, every_other_vars, gap2_vars, block_deviation_vars,
                                 balanced_full_wkends_off_deviation_vars, different_weekend_duty_day_vars)

    model.Maximize(full_obj)
    solver = solveModel(model)
    exportToExcel(solver, x, doctors_list, dates)
    printStatistics(solver, x, doctors_list, dates, is_weekend)
