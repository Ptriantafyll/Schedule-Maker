...
Comprehensive integration reference for frontend and mobile engineers connecting client applications (Flutter Mobile, Flutter Web, SPAs) to the **Schedule-Maker** backend API.
...
1. **Native / Mobile Mode (Flutter Android, iOS, Desktop)**:
   ...
   - Refresh tokens are stored securely in hardware-backed storage (Keystore / Keychain) and exchanged in JSON request/response bodies.
...
- `access_token`: Short-lived (15–30 minutes). Can be kept in-memory or secure storage.
...
To prevent Cross-Site Request Forgery (CSRF), state-changing refresh and logout calls require the **Double-Submit CSRF** pattern:

1. When logging in or refreshing, the server sets:
   - `refresh_token`: `HttpOnly`, `SameSite=Lax`, `Secure` (cannot be read by JavaScript).
   - `csrf_token`: `SameSite=Lax`, `Secure` (readable by JavaScript).
...
- **Length**: Between **10** and **72** UTF-8 bytes (72 bytes is the strict bcrypt truncation boundary).
- **Character Diversity**: Must contain at least:
  - 1 uppercase letter (`A-Z`)
  - 1 lowercase letter (`a-z`)
  - 1 digit (`0-9`)
  - 1 special symbol (`!@#$%^&*...`)
- **Blocklist**: Common passwords (e.g. `Password123!`, `Admin1234!`) are rejected even if they meet diversity criteria.

Frontend clients should perform client-side pre-validation using identical regex rules to provide real-time UI feedback.
...
| `401 Unauthorized` | Invalid/expired access token or credentials | Attempt silent refresh; redirect to login if refresh fails |
| `403 Forbidden` | Authenticated user lacks permission | Display "Access Denied" or hide restricted UI controls |
